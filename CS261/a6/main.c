...
int main (int argc, const char * argv[]) {
	...
	ValueType getVal;
//	ValueType one = (int *)malloc(sizeof(int));
//	(*one) = 1;
	...
	currentWord = getWord(fileptr);


	//Might need to change this to end of file
	while(!feof(fileptr)) {

		if(containsKey(hashTable, currentWord) == 0) {
			getVal = (int *)malloc(sizeof(int));
			*getVal = 1;
			insertMap(hashTable, currentWord, getVal);
		}
		
		else {
		//	(*getVal) = *(atMap(hashTable, currentWord)) + 1;
			getVal = atMap(hashTable, currentWord);
			(*getVal)++;
			insertMap(hashTable, currentWord, getVal);
		}
