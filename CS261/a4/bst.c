...
struct Node *allocateNewNode(TYPE val) {
...
{
    /*write this*/
	struct Node* traverse = cur;
	assert(val != NULL);
	if(cur == NULL) {
		cur = allocateNewNode(val);
	}
	
	while(traverse != NULL) {
		if(val == traverse->val) break;

		else if(compare(val, traverse->val) == -1) {
	
			if(traverse->left == NULL) {
				traverse->left = allocateNewNode(val);
				break;
			}
			...
			if(traverse->right == NULL) {
				traverse->right = allocateNewNode(val);
				break;
			}
...
{
    /*write this*/
	struct Node* search = tree->root;

	while(search != NULL) {
		if(compare(val, search->val) == 0)
			return 1;
		else if(compare(val, search->val) == -1) {
			search = search->left;
		}
		else
			search = search->right;
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if(temp->left != NULL)
		temp->left = _removeLeftMost(temp->left);

	else if(temp->right != NULL) {
		temp->val = _leftMost(temp->right);
		temp->right = _removeLeftMost(temp->right);
	}
...
void testLeftMost() {
    ...
    struct data myData3, myData4;
     ...
     //ARE THESE MEANT TO BE REMINDERS?
    myData3.number = 110;
    myData3.name = "righty";
    myData4.number = 10;
    myData4.name = "lefty of lefty";
...
void betterTest() {
	...
	printf("Leftmost of tree is currently: ");
	print_type(_leftMost(tree->root));

	printf("\nLeft of root is: ");
	print_type(cur->left->val);
	printf("\nLeft of left root is: ");
	print_type(cur->left->left->val);

//	printf("\nDeleting leftmost node...");
	cur = _removeLeftMost(tree->root);
	
	printf("\nLeftmost of tree is now: ");
	print_type(_leftMost(cur));
//	printf("\n");
}
