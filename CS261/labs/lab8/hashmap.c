...
struct hashLink* allocateNewLink(char* key, int value) {
...
void insertMap(struct hashMap *map, char *key, int value) {
	...
	// Step 1: Compute hash for the key
	
	int bucketIndex;
	struct hashLink* traverse;

	//Step 2: Determine the bucket in the hash map this key belongs in
	bucketIndex = _hashString(key) % map->tableSize;

	//If bucket has no items in it
	if(map->table[bucketIndex] == NULL) {
		map->table[bucketIndex] = allocateNewLink(key, value);
		map->count++;
	
	}
	//If bucket has entries in it
	else{
		traverse = map->table[bucketIndex];
		while(traverse != NULL) {
			traverse = traverse->next;
		}
		traverse = allocateNewLink(key, value);
		map->count++;
	}
...
int containsKey(struct hashMap *map, char *key) {
	int retval = 0;
	int bucketIndex;
	struct hashLink* traverseBucket;
	
	bucketIndex = _hashString(key) % map->tableSize;

	traverseBucket = map->table[bucketIndex];
	
	while(traverseBucket != NULL) {
		if(strcmp(traverseBucket->key, key) == 0){
			retval = 1;
			break;
		}
		traverseBucket = traverseBucket->next;
	}
...
double tableLoad(struct hashMap *map) {
    // FIXME: You get to implement this.
    double ratio = map->count / map->tableSize;
    return ratio;
}
