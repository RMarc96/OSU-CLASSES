...
void removeMinHeap(DynArr *heap)
{
  ...
  /* Rebuild heap */
  _adjustHeap(heap, last, 0); /* Adjust up to , but not including last, which is now one beyond end */
}
...
void sortHeap(DynArr *heap)
{
  ...
  for (i = sizeDynArr(heap) - 1; i > 0; i--) {
	  swapDynArr(heap, 0, i);
	  _adjustHeap(heap, i, 0);  /* don't include the last spot which is now in correct sorted location*/
  }
}
