...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
  /* FIXME Write this */
	...
	TYPE left = getDynArr(heap, i);
	TYPE right = getDynArr(heap, j);	

	smaller = compare(left, right);
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
	...
	int childIndex = sizeDynArr(heap);
	int parentIndex;	
	TYPE parentVal; 
	TYPE childVal;
	...
	//After adding the node, we need to percolate if necessary
	while(childIndex != 0) {
		parentIndex = ( childIndex - 1) / 2;
		childVal = getDynArr(heap, childIndex);
		parentVal = getDynArr(heap, parentIndex); 

		if(compare(childVal, parentVal) == -1) {
			swapDynArr(heap, childIndex, parentIndex);
...
{
  /* FIXME: Write this */
	//Given pos, we can find child nodes through the equation 2i+1 and 2i+2
	int leftIndex = pos * 2 + 1;	
	int rightIndex = pos * 2 + 2;
	int minIndex;

	if(rightIndex < max) {
		minIndex = _smallerIndexHeap(heap, leftIndex, rightIndex, compare);	
		
		if(compare(getDynArr(heap, minIndex), getDynArr(heap, pos)) == -1) {
			swapDynArr(heap, minIndex, pos);
			_adjustHeap(heap, max, minIndex, compare);
		}
	}

	else if(leftIndex < max) {
		if(compare(getDynArr(heap, leftIndex), getDynArr(heap, pos)) == -1) {
			swapDynArr(heap, leftIndex, pos);
			_adjustHeap(heap, max, leftIndex, compare);
		}
...
void removeMinHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
	assert(heap != NULL);
	swapDynArr(heap, sizeDynArr(heap) - 1, 0);
...
{
  /* FIXME: Write This */
	//All this function does is it rearranges any DA to a heap-structure

	int i;	
	int maxSize = sizeDynArr(heap);
	int nonLeafIndex; 

	nonLeafIndex = maxSize / 2 - 1;

	//Must decrement until we reach root
	for(i = nonLeafIndex; i >= 0; i--) {
		_adjustHeap(heap, maxSize, i, compare);
	}
...
void sortHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
	int i;
	int maxSize = sizeDynArr(heap);
	//First build DA into heap, if not already
	_buildHeap(heap, compare);		


	for(i = maxSize - 1; i > 0; i--) {
		//Then swap first and last elements
		swapDynArr(heap, 0, sizeDynArr(heap) - 1);
		//Then adjust heap (first to last [not inclusive])
