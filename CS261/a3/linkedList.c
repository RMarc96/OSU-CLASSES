...
void _printList(struct linkedList* lst)
{
	/* FIXME: you must write this */
	assert(lst != NULL);
	struct DLink* traverse = lst->firstLink->next;

	printf("Current list: [ ");
	while(traverse != lst->lastLink) {
...
int containsList (struct linkedList *lst, TYPE e) {
	...
	assert(lst != NULL);
	//assert(!isempty)
	struct DLink* traverse = lst->firstLink;

	while(!EQ(traverse, lst->lastLink->next)) {
		if(EQ(traverse->value, e))
			return 1;
...
void removeList (struct linkedList *lst, TYPE e) {
	...
	while(!EQ(traverse, lst->lastLink)) {
		if(EQ(traverse->value, e)) {
			_removeLink(lst, traverse);
			break;
		}	
		traverse = traverse->next;
	}
