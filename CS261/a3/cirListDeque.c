...
void printCirListDeque(struct cirListDeque *q)
{
	/* FIXME: you must write this */
	int i;
	struct DLink* traverse = q->Sentinel->next;

	printf("Current deque: [ ");

	for(i = 0; i < q->size; i++) {
		printf("%f ", traverse->value);
		traverse = traverse->next;
	}
	printf("]\n");


//	while(traverse != q->Sentinel) {
//		printf("%f  ", traverse->value);
...
void reverseCirListDeque(struct cirListDeque *q)
{
	...
	int i;
	TYPE swapVal;
	struct DLink* forward = q->Sentinel->next;
	struct DLink* backward = q->Sentinel->prev;	
	
			
	for(i = 0; i < q->size / 2; i++) {
		swapVal = forward->value;
		forward->value = backward->value;
		backward->value = swapVal;
