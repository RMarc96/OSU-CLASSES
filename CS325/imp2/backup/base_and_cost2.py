...
#		print matrix[0, j]
	...
	for i in range(1, (len(str1)+1)):
		for j in range(1, (len(str2)+1)):
			...
			if matrix[i-1,j]+getCost(str1[i-1], "-") < matrix[i, j-1]+getCost("-", str2[j-1]) and matrix[i-1,j]+getCost(str1[i-1], "-") < matrix[i-1, j-1]+cost:	
				flag = 1 #Then we performed a deletion
			elif matrix[i, j-1]+getCost("-", str2[j-1]) < matrix[i-1,j]+getCost(str1[i-1], "-") and matrix[i, j-1]+getCost("-", str2[j-1]) < matrix[i-1, j-1]+cost:
				flag = 2 #Then we performed an insertion
			else:
				flag = 3 #Then we performed an alignment
			print flag
